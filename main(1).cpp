...
std::string vote_majority(const std::vector<QueryResult>& nearest_points, int k) {
    ...
    for (int i = 0; i < k; ++i) {
        std::string state_county = nearest_points[i].state_alpha + " " + nearest_points[i].county_name;
...
int main() {
    ...
    std::cout << "The majority-voted state and county for the given coordinates are: " << majority_state_county << std::endl;
}